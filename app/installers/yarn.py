...
def install_yarn():
    try:
        ...
        subprocess.run(["pkexec", "/usr/bin/npm", "install", "-g", "yarn"], check=True, capture_output=True)
...
def uninstall_yarn():
    try:
        ...
        subprocess.run(["pkexec", "/usr/bin/npm", "uninstall", "-g", "yarn"], check=True, capture_output=True)
